...
def run(engine, aoi_id: str, cb=None) -> int:
    ...
    # 3) Schéma + table results (id, aoi_id + LIKE geo.arrachage_vignes)
    with engine.begin() as conn:
        conn.execute(text("CREATE SCHEMA IF NOT EXISTS ecocompensation_results;"))
        conn.execute(text("DROP TABLE IF EXISTS ecocompensation_results.arrachage_vignes CASCADE;"))
        conn.execute(
            text(
                """
                CREATE TABLE ecocompensation_results.arrachage_vignes (
                ...
                CREATE INDEX IF NOT EXISTS idx_ecocomp_results_arrachage_aoi
                    ON ecocompensation_results.arrachage_vignes (aoi_id);
                """
            )
        )
    ...
    with engine.begin() as conn:
        res = conn.execute(
            text(
                f"""
                INSERT INTO ecocompensation_results.arrachage_vignes (aoi_id, {cols_str})
                SELECT :aoi_id AS aoi_id, {select_cols_str}
                FROM geo.arrachage_vignes a
                WHERE ST_Intersects(
                    a.geom_2154,
                    ST_GeomFromText(:wkt_aoi, 2154)
